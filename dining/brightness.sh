...
do
    ...
    lux=$(curl -k http://pindadomo/data/luxtls)
    if [ $? = 0 ]; then # Ophalen van meting gelukt en kan dus verwerkt worden
        echo "Omgevingslicht: $lux"
        maxlux=$(curl -k http://pindadomo/data/luxmaxtls)
        minlux=$(curl -k http://pindadomo/data/luxmintls)
        ...
        helderheid=$(awk "BEGIN {printf \"%.0f\", 0 + $rellux * 100}") # from 0 to 100
